...
from model.decoder import FloorDecoder
...
def sample(args, cfg):
    ...
    floor_decoder.load_state_dict(torch.load(os.path.join(args.sketch_weight, 'pytorch_model.bin')))
    floor_decoder = floor_decoder.to(device).eval()
    ...
    for i in range(OUT_SAMP_NUM):
        print(f'Generate Floor Plan %d...'%i)
        ...
        while len(generated_svg) < NUM_SAMPLE:
            sample_pixels = floor_decoder.sample(n_samples=BS, text='')
            generated_svg.append(sample_pixels)
        end_time = time.time()
        print(f'Generate {len(generated_svg)} svg in {end_time - start_time} seconds')
        for i, svg in enumerate(generated_svg):
            xxx = np.array(svg[0])
            print(xxx.shape)
            np.savetxt(output_dir+'/'+str(i)+'.txt', xxx)
        # convert token sequence into SVG
        # print('Rendering...')
        # gen_data = []

        # tmp_c = 0
        # print(len(generated_svg))
