...
def train(args, cfg):
    ...
    train_dataset = SketchData(args.train_metadata, args.svg_folder, args.img_folder, args.maxlen, cfg['bdry_len'], tokenizer, require_aug=False)
    ...
    val_dataset = SketchData(args.valid_metadata, args.svg_folder, args.img_folder, args.maxlen, cfg['bdry_len'], tokenizer, require_aug=False)
    ...
    model = SketchDecoder(
        ...
        pix_len=train_dataset.maxlen,
        bdry_len=cfg['bdry_len'],
    ...
    for epoch in range(starting_epoch, cfg['epoch']):
        ...
        total_loss, total_pix_loss, total_bdry_loss = 0., 0., 0.

        for pix, mask, bdry, bdry_mask in train_dataloader:
            with accelerator.accumulate(model):
                ...
                # print('bdry:', bdry)
                # print('bdry shape:', bdry.shape)
                # print('bdry_xy', bdry_xy)
                # print('bdry_xy shape', bdry_xy.shape)
                loss, pix_loss, bdry_loss = model(pix, mask, bdry, bdry_mask, return_loss=True)
                total_loss += loss.item() / accum_step
                total_pix_loss += pix_loss.item() / accum_step
                total_bdry_loss += bdry_loss.item() / accum_step
            ...
            if accelerator.sync_gradients and accelerator.is_local_main_process:
                if overall_step % cfg['log_every'] == 0:
                    ...
                    writer.add_scalar("loss/bdry_loss", total_bdry_loss, overall_step)
        ...
        if (epoch+1) % cfg['val_every'] == 0:
            ...
            with tqdm(val_dataloader, unit="batch", disable=not accelerator.is_local_main_process) as batch_data:
                for pix, mask, bdry, bdry_mask in batch_data:
                    with torch.no_grad():
                        loss, pix_loss, bdry_loss = model(pix, mask, bdry, bdry_mask, return_loss=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--valid_metadata", type=str, required=True)
    ...
    config = {
        ...
        'num_heads': 8,
        'dropout_rate': 0.1,
        'word_emb_path': 'pretrained/word_embedding_512.pt',
        'pos_emb_path': None,
        'gradient_accumulation_steps': 1,

        'lr': 6e-4,  # need scaling
        'warm_up_steps': 4000,
        'epoch': 300,
